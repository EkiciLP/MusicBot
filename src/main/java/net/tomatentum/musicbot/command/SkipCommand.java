...
public class SkipCommand extends GuildCommand {
    ...
    public SkipCommand() {
        super("skip", "Skip the specified seconds", new OptionData(OptionType.INTEGER, "time", "Time in Seconds", true));
    }
    ...
    public void execute(SlashCommandEvent command) {
        ...
        musicManager.skip(time);

        command.getHook().sendMessage(String.format("Skipped %s seconds to %s", time, TomatenMusic.getTimestamp(musicManager.getPlayer().getPlayingTrack().getPosition()))).queue();
